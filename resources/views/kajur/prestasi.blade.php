...
<div class="mb-8 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
    ...
    <a href="{{ route('kajur.prestasi.create') }}" class="inline-flex items-center gap-2 bg-[#006633] text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-md shadow-green-200 hover:bg-[#004d26] transition-all">
...
<div class="w-full min-w-0 bg-white rounded-3xl border border-gray-100 shadow-sm flex flex-col mb-8 overflow-hidden">
    ...
    <form id="filter-form" action="{{ route('kajur.prestasi') }}" method="GET">
        ...
        <div class="p-4 sm:p-6 border-b border-gray-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gray-50/30">
            ...
            <div class="relative w-full sm:w-max flex gap-2">
                ...
                <a href="{{ route('kajur.prestasi') }}" class="px-3 py-2 bg-red-50 text-red-500 rounded-lg text-sm font-bold hover:bg-red-500 hover:text-white transition-all tooltip" title="Reset Semua Filter">
                ...
                <tr class="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td class="px-4 py-4 whitespace-nowrap align-top text-center">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            <a href="{{ route('kajur.prestasi.show', $item->id) }}" class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-500 hover:text-white flex items-center justify-center transition-colors tooltip" title="Detail"><i class="bi bi-eye-fill"></i></a>

                            @if($item->status === 'rejected')
                                {{-- KHUSUS REJECTED: Cuma bisa ubah status --}}
                                <form action="{{ route('kajur.prestasi.status-update', $item->id) }}" method="POST" class="inline">
                                    @csrf @method('PATCH')
                            ...
                            @else
                                ...
                                <a href="{{ route('kajur.prestasi.edit', $item->id) }}" class="w-8 h-8 rounded-lg bg-yellow-50 text-yellow-600 hover:bg-yellow-400 hover:text-white flex items-center justify-center transition-colors tooltip" title="Edit"><i class="bi bi-pencil-square"></i></a>
                                
                                <form action="{{ route('kajur.prestasi.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Hapus permanen data prestasi ini?')">
                                    @csrf @method('DELETE')
