...
<div class="mb-8 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
    ...
    <div class="flex items-center gap-3">
        <a href="{{ route('kajur.prestasi.edit', $prestasi->id) }}" class="inline-flex items-center gap-2 bg-yellow-50 text-yellow-600 px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-yellow-400 hover:text-white transition-all">
            <i class="bi bi-pencil-square"></i> Edit Data
        </a>
        <form action="{{ route('kajur.prestasi.destroy', $prestasi->id) }}" method="POST" onsubmit="return confirm('Hapus permanen data prestasi ini?')">
            @csrf @method('DELETE')
