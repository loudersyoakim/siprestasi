...
<div class="mb-8 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
    ...
    <div class="flex items-center gap-3">
        <a href="{{ route('admin.prestasi.edit', $prestasi->id) }}" class="inline-flex items-center gap-2 bg-yellow-50 text-yellow-600 px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-yellow-400 hover:text-white transition-all">
            <i class="bi bi-pencil-square"></i> Edit Data
        </a>
        <form action="{{ route('admin.prestasi.destroy', $prestasi->id) }}" method="POST" onsubmit="return confirm('Hapus permanen data prestasi ini?')">
            @csrf @method('DELETE')
            <button type="submit" class="inline-flex items-center gap-2 bg-red-50 text-red-600 px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-red-500 hover:text-white transition-all">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="space-y-8">
        ...
        <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden p-6">
            ...
            <div class="space-y-5">
                <div>
                    <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Validasi Admin</div>
