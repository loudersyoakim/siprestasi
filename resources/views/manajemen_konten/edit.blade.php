...
<div class="mb-8 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
    <div>
        ...
        <h3 class="text-2xl font-black text-gray-800 tracking-tight mt-1">Edit Konten</h3>
    </div>
</div>

<div class="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden mb-8">
    <form action="{{ route('konten.update', $konten->id) }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-10 space-y-8 max-w-4xl mx-auto">
        @csrf @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="space-y-2">
                ...
                <select name="kategori" required class="w-full px-5 py-3.5 bg-gray-50 border-none rounded-2xl text-sm focus:ring-2 focus:ring-[#006633]/20 focus:bg-white outline-none font-bold transition-all cursor-pointer">
                    ...
                    <option value="informasi" {{ $konten->kategori == 'informasi' ? 'selected' : '' }}>Informasi</option>
                    <option value="pengumuman" {{ $konten->kategori == 'pengumuman' ? 'selected' : '' }}>Pengumuman</option>
                    <option value="prestasi" {{ $konten->kategori == 'prestasi' ? 'selected' : '' }}>Prestasi</option>
                </select>
            </div>
            
            <div class="space-y-2">
                <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Ganti Thumbnail</label>
                <div class="flex items-center gap-4">
                    {{-- Kotak Preview (Ambil dari DB atau Placeholder) --}}
                    <div class="w-16 h-14 rounded-xl bg-gray-100 border border-gray-200 overflow-hidden shrink-0 shadow-inner">
                        <img id="image-preview" src="{{ $konten->gambar_cover ? asset('storage/'.$konten->gambar_cover) : 'https://ui-avatars.com/api/?name=Img&background=f3f4f6&color=9ca3af' }}" class="w-full h-full object-cover">
                    </div>
                    {{-- Input File --}}
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-[9px] text-gray-400 italic mt-1.5">*Abaikan jika tidak ingin diganti.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-2">
            ...
            <textarea name="isi_konten" id="editor">{{ old('isi_konten', $konten->isi_konten) }}</textarea>
        ...
        <div class="flex items-center justify-between p-6 bg-gray-50 rounded-2xl border border-gray-100">
            <div>
                <h4 class="text-sm font-bold text-gray-800">Status Publikasi</h4>
                <p class="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Saat ini: {{ $konten->is_aktif ? 'Live' : 'Draft' }}</p>
            </div>
            <label class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" name="is_aktif" value="1" class="sr-only peer" {{ $konten->is_aktif ? 'checked' : '' }}>
