    ...
    <tr class="hover:bg-gray-50/50 transition-colors group">
        ...
        <td class="px-6 py-4 align-top">
            @switch(trim($user->role))
                @case('super_admin') <span class="px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-white bg-[#006633] rounded-lg shadow-sm">Super Admin</span> @break
                ...
                @case('wakil_dekan') <span class="px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-purple-600 bg-purple-100 rounded-lg">Wakil Dekan</span> @break
                @case('jurusan') <span class="px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-orange-600 bg-orange-100 rounded-lg">Kajur</span> @break
        ...
        <td class="px-6 py-4 align-top text-center">
            <div class="flex items-center justify-center gap-2">
                ...
                {{-- LOGIKA TOMBOL HAPUS --}}
                @php
                    // Hitung jumlah super admin yang ada di sistem
                    $totalSuperAdmin = \App\Models\User::where('role', 'super_admin')->count();
                    // Cek apakah user saat ini adalah user yang sedang login
                    $isSelf = Auth::id() === $user->id;
                    // Cek apakah user ini adalah super admin terakhir
                    $isLastSuperAdmin = ($user->role === 'super_admin' && $totalSuperAdmin <= 1);
                @endphp

                {{-- Tombol hapus muncul jika:
                     1. Bukan diri sendiri
                     2. Bukan Super Admin terakhir yang tersisa di sistem
                     3. Jika login sebagai Admin, dilarang hapus Super Admin --}}
                ...
                @elseif($isLastSuperAdmin)
                    <span class="w-8 h-8 flex items-center justify-center text-gray-300 cursor-not-allowed" title="Super Admin Terakhir tidak bisa dihapus">
