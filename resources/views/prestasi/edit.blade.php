...
<div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden mb-8">
    ...
    <form action="{{ route('prestasi.update', $prestasi->id) }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6" onsubmit="return validateForm()">
        @csrf @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {{-- BAGIAN KEPESERTAAN & PELAPOR (PRELOAD) --}}
            <div class="md:col-span-2 p-6 bg-blue-50/50 border border-blue-100 rounded-3xl space-y-6">
                ...
                <div class="relative" id="student-search-container">
                    ...
                    <div class="relative mt-1">
                        ...
                        <input type="text" id="student-search-input" placeholder="Ketik Nama atau NIM..." autocomplete="off" class="w-full pl-11 pr-4 py-3 bg-white border border-gray-200 rounded-2xl text-sm focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 outline-none transition-all shadow-sm">
                    </div>
                    ...
                    <div id="student-dropdown" class="absolute z-[70] w-full mt-2 bg-white rounded-2xl border border-gray-100 shadow-2xl overflow-hidden hidden">
                        <div class="max-h-60 overflow-y-auto custom-scrollbar" id="student-list">
                            ...
                            <div class="student-item px-4 py-3 hover:bg-yellow-50 cursor-pointer border-b border-gray-50 transition-colors" data-id="{{ $mhs->id }}" data-name="{{ $mhs->name }}" data-nim="{{ $mhs->nim_nip }}">
                ...
                <div>
                    ...
                    <div id="selected-students-container" class="flex flex-col gap-2 mt-2 min-h-[60px] p-4 bg-white border border-dashed border-blue-200 rounded-2xl shadow-sm"></div>
            ...
            <div class="{{ in_array($field->tipe, ['textarea']) ? 'md:col-span-2' : '' }}">
                ...
                @if($field->tipe === 'text' || $field->tipe === 'number' || $field->tipe === 'date')
                    <input type="{{ $field->tipe }}" name="{{ $inputName }}" value="{{ is_array($oldValue) ? implode(', ', $oldValue) : $oldValue }}" {{ $field->is_required ? 'required' : '' }} class="w-full px-4 py-3 border border-gray-200 rounded-2xl text-sm focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 outline-none transition-all font-medium">
                
                @elseif($field->tipe === 'textarea')
                    <textarea name="{{ $inputName }}" rows="3" {{ $field->is_required ? 'required' : '' }} class="w-full px-4 py-3 border border-gray-200 rounded-2xl text-sm focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 outline-none transition-all font-medium">{{ $oldValue }}</textarea>
                    ...
                    <select name="{{ $inputName }}" {{ $field->is_required ? 'required' : '' }} class="condition-trigger w-full px-4 py-3 border border-gray-200 rounded-2xl text-sm focus:border-yellow-500 outline-none bg-white font-medium">
                        ...
                        @if(is_array($field->opsi))
                            @foreach($field->opsi as $opsi) 
                                <option value="{{ trim($opsi) }}" {{ $oldValue == trim($opsi) ? 'selected' : '' }}>{{ trim($opsi) }}</option> 
                            @endforeach
                        @endif
                    </select>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateSelectedUI() {
            ...
            if (selectedUsers.length === 0) {
                selectedContainer.innerHTML = '<div class="text-gray-400 text-xs italic m-auto">Belum ada mahasiswa yang dipilih...</div>';
                return;
            }
            ...
            selectedUsers.forEach((user, index) => {
                ...
                chip.className = `flex items-center justify-between p-3 rounded-xl border ${isKetua ? 'bg-yellow-50 border-yellow-200' : (user.isManual ? 'bg-orange-50 border-orange-200' : 'bg-gray-50 border-gray-200')} shadow-sm`;
                
                chip.innerHTML = `
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 rounded-full ${isKetua ? 'bg-yellow-500 text-white' : 'bg-gray-200 text-gray-600'} flex items-center justify-center font-black text-xs">
                            ${user.name.substring(0,1).toUpperCase()}
                        </div>
                        <div class="flex flex-col">
                            <span class="text-sm font-bold ${isKetua ? 'text-yellow-800' : 'text-gray-800'}">${user.name}</span>
                            <span class="text-[9px] ${user.isManual ? 'text-orange-600' : 'text-gray-500'} font-black uppercase tracking-widest">
                                ${user.isManual ? 'Input Manual' : 'NIM: ' + user.nim}
                            </span>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        ${isKetua ? '<span class="px-2 py-1 bg-yellow-500 text-white text-[9px] font-black uppercase rounded-lg">Ketua / Pelapor</span>' : '<span class="px-2 py-1 bg-gray-200 text-gray-500 text-[9px] font-black uppercase rounded-lg">Anggota</span>'}
        ...
        window.validateForm = function() {
            if(selectedUsers.length === 0) { alert('Pilih minimal 1 Mahasiswa!'); return false; }
            return true;
        }
