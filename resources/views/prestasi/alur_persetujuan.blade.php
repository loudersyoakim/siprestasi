...
<div class="mb-8 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
    <div>
        ...
        <h3 class="text-2xl font-black text-gray-800 tracking-tight">Manajemen Alur Persetujuan</h3>
    </div>
</div>
...
<div class="flex flex-col gap-8">
    ...
    {{-- BAGIAN ATAS: Banner Penjelasan & Info --}}
    <div class="bg-blue-50/50 border border-blue-100 rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row md:items-center gap-6 shadow-sm">
        <div class="w-16 h-16 shrink-0 bg-blue-100 text-blue-600 rounded-2xl flex items-center justify-center text-3xl">
            <i class="bi bi-diagram-3-fill"></i>
        </div>
        <div class="flex-1">
            <h4 class="text-base font-black text-blue-900 mb-1">Birokrasi Otomatis</h4>
            <p class="text-sm text-blue-700 leading-relaxed mb-4 max-w-4xl">
                Tentukan tahapan validasi yang harus dilalui oleh setiap data prestasi mahasiswa sebelum statusnya dinyatakan <b>Approved (Disetujui)</b> dan siap dipublikasi.
            </p>
            <div class="flex flex-col sm:flex-row gap-4 sm:gap-8">
                <div class="flex items-start gap-2">
                    <i class="bi bi-check-circle-fill text-green-500 mt-0.5"></i>
                    <span class="text-blue-800 ">Jika dinonaktifkan, tahapan tersebut akan dilewati (Bypass).</span>
                </div>
                <div class="flex items-start gap-2">
                    <i class="bi bi-info-circle-fill text-yellow-500 mt-0.5"></i>
                    <span class="text-blue-800 ">Data dari Admin/Staff otomatis <b>Approved</b> tanpa alur ini.</span>
                </div>
            </div>
        </div>
    </div>

    {{-- BAGIAN BAWAH: Timeline Konfigurasi --}}
    <div>
        <form action="{{ route('prestasi.alur.update') }}" method="POST" class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 sm:p-8 relative">
            ...
            <div class="mb-8 border-b border-gray-50 pb-4 flex justify-between items-center">
                ...
                <button type="submit" class="px-6 py-2.5 bg-[#006633] text-white text-xs font-bold uppercase tracking-widest rounded-xl hover:bg-black transition-all shadow-lg shadow-green-900/20">
                    Simpan Alur
                </button>
            </div>

            {{-- TIMELINE CONTAINER --}}
            <div class="relative pl-4 sm:pl-8 max-w-3xl mx-auto">
                ...
                <div class="relative flex items-start gap-6 mb-10 group">
                    ...
                    <div class="flex-1 {{ $item->is_active ? 'bg-green-50/30 border-[#006633]/20 shadow-sm' : 'bg-white border-gray-100 opacity-60' }} border rounded-2xl p-5 transition-all duration-300 timeline-card hover:shadow-md">
